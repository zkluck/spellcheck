 ...
 * @param id 唯一测试ID
 */
async function setScenarioCookies(page: Page, scenario: string, id: string): Promise<void> {
 ...
 * @param page Playwright页面对象
 */
async function runOnce(page: Page): Promise<void> {
  await page.goto('/');
 ...
 * @param page Playwright页面对象
 */
async function waitFinishWithoutError(page: Page): Promise<void> {
  // 等待按钮回到"开始检测"，表示流程结束
